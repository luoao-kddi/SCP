...
from dataloaders.encode_dataset_ehem import EncodeEHEMDataset
from models import *
...
def compress_ehem_xyz(batch, outputfile, model, args):
    model.eval()

    context_size = model.cfg.model.context_size

    ids, pos, xyz_pos, data, oct_seq, pt_num, pc, bin_num, z_offset = batch
    ...
    with torch.no_grad():
        ...
        for l, (level_data, level_pos, level_xyz_pos, level_ids) in enumerate(zip(data, pos, xyz_pos, ids)):
            level_data, level_pos, level_xyz_pos, level_ids = level_data.cuda(), level_pos.cuda(), level_xyz_pos.cuda(), level_ids.cuda()
            probabilities = torch.zeros((level_data.shape[1], model.cfg.model.token_num)).cuda()
            for i in tqdm.trange(0, level_data.shape[1], interval, desc=f"level {l} of {len(data)}"):
                ipt_data = level_data[:, i : i + context_size].long()
                ipt_pos = level_pos[:, :, i : i + context_size]
                ipt_xyz_pos = level_xyz_pos[:, :, i : i + context_size]
                node_id = level_ids[0, i : i + context_size]
                start_time = time.time()
                output1, output2 = model(ipt_data, ipt_pos, ipt_xyz_pos, enc=True)
                elapsed = elapsed + time.time() - start_time
...
def main(args):
    ...
    if 'EHEM' in model_name:
        testset = EncodeEHEMDataset(test_files, model.cfg.model.context_size, args.type, args.level_wise, args.lidar_level, args.cylin, args.spher or args.spher_circle, args.spher_circle, cfg.data.extra_pos, args.preproc_path)
    else:
    ...
    for i, (cur_file, batch) in enumerate(zip(test_files, test_loader)):
        ...
        print(psnr[-1], bpps[-1], chamfer[-1], times[-1])
